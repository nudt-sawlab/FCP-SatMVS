...
依赖说明
--------
本仓库包含网络与算法核心实现，**不包含**数据集加载脚本。运行训练/预测前需要
自行准备 ``dataset`` 包，并向 ``find_dataset_def`` 注册数据集。期望的接口：

    from dataset import find_dataset_def         # 返回数据集类
    from dataset.data_io import save_pfm         # 仅在保存深度图时使用

数据集类签名应为：``MVSDataset(root, mode, view_num, ref_view, use_qc)``，
返回的样本字典需包含：``imgs``、``cam_para``、``depth_values``、``depth``、
``mask``、``out_view``、``out_name``。详情可参考 WHU-TLC 数据集实现。
"""
...
if args.resume:
    ...
    assert not args.loadckpt, "--resume 不能与 --loadckpt 同时使用"
if testpath is None:
    testpath = trainpath
