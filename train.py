"""FCP-SatMVS training entry point.

Dataset note
------------
This repository contains the network and algorithmic core but **does not**
ship a dataset loader. Users must provide a ``dataset`` package that exposes
the following interface::

    from dataset import find_dataset_def         # returns a dataset class
    from dataset.data_io import save_pfm         # used when saving depth maps

The dataset class signature is::

    MVSDataset(root, mode, view_num, ref_view, use_qc)

and each sample dict must contain ``imgs``, ``cam_para``, ``depth_values``,
``depth``, ``mask``, ``out_view`` and ``out_name`` (see the WHU-TLC reference
...
from tools.utils import (
    AbsDepthError_metrics, DictAverageMeter, Thres_metrics, make_nograd_func,
    print_args, save_images, save_scalars, tensor2float, tensor2numpy, tocuda,
)

cudnn.benchmark = True


parser = argparse.ArgumentParser(description='FCP-SatMVS Training')
parser.add_argument('--mode', default='test', choices=['train', 'test', 'profile'])
parser.add_argument('--model', default="casmvs", choices=["casmvs"], help='select model')
parser.add_argument('--geo_model', default="pinhole", choices=["rpc", "pinhole"], help='geometry model')
parser.add_argument('--use_qc', default=False, help='whether to use Quaternary Cubic Form for RPC warping.')
parser.add_argument('--dataset_root', default='/mnt/e/SatMVS/WHU-TLC', help='dataset root')

parser.add_argument('--loadckpt', default="", help='load a specific checkpoint')
parser.add_argument('--logdir', default='./checkpoints', help='checkpoint/log directory')
parser.add_argument('--resume', default=False, help='continue training from latest checkpoint')
...
if args.resume:
    ...
    assert not args.loadckpt, "--resume cannot be used together with --loadckpt"
if testpath is None:
    testpath = trainpath
...
if (args.mode == "train" and args.resume) or (args.mode == "test" and not args.loadckpt):
    saved_models = [fn for fn in os.listdir(cur_log_dir)
                    if fn.endswith(".ckpt") and len(fn.split("_")) == 3]
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            loss, scalar_outputs, image_outputs = train_sample(
                sample, lr_scheduler, detailed_summary=do_summary,
            )
            if do_summary:
                save_scalars(logger, 'train', scalar_outputs, global_step)
                save_images(logger, 'train', image_outputs, global_step)
            print('Epoch {}/{}, Iter {}/{}, train loss = {:.3f}, time = {:.3f}, train_result = {}'.format(
        ...
        with open(cur_log_dir + '/train_record.txt', "a+") as train_record:
...
def test():
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        print("Iter {}/{}, {}, time = {:3f}, test results = {}".format(
            batch_idx, len(TestImgLoader), bname, time.time() - start_time, scalar_outputs))
        ...
        mask = (np.squeeze(tensor2numpy(mask))).astype(np.int32)
...
def train_sample(sample, lr_scheduler, detailed_summary=False):
    ...
    image_outputs = {
        "depth_est": depth_est, "depth_gt": depth_gt,
        "ref_img": sample["imgs"][:, 0], "mask": sample["mask"]["stage1"],
    }
